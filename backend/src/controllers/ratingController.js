...
const addRating = async (req, res) => {
  try {
    ...
    // duplicate check scoped to this target, matching the unique index
    ...
    return res.status(201).json({ msg: "Rating submitted successfully", rating: newRating });
  } catch (error) {
    if (error.code === 11000) {
      return res.status(400).json({ msg: "You have already rated this" });
    }
...
const getAllRatings = async (req, res) => {
  try {
    ...
    const summaryMap = {};
    ratings.forEach(r => {
      const key = `${r.targetType}:${r.targetName}`; // group by name+type, not a nonexistent targetId
      if (!summaryMap[key]) {
        summaryMap[key] = {
    ...
    return res.status(200).json({ msg: "All ratings fetched", total: ratings.length, summary, ratings });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
...
const getRatingsByTarget = async (req, res) => {
  try {
    ...
    const { targetType } = req.query; // optional, in case names collide across types

    const filter = { targetName, isVisible: true };
...
const toggleVisibility = async (req, res) => {
  try {
    ...
    return res.status(200).json({ msg: `Rating ${rating.isVisible ? "shown" : "hidden"} successfully`, rating });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ msg: "Internal Server Error" });
  }
