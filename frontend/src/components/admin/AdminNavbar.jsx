...
function AdminNavbar({ onToggleSidebar }) {
  ...
  const navLinks = [
    ...
    { to: "/admin/dashboard/PlacedStudentDashboard", label: "Placements" },
    { to: "/admin/dashboard/AllEventRegistration", label: "All Event Registration" },
    { to: "/admin/dashboard/EnrolledStudent", label: "Enrolled Student" },
    { to: "/admin/dashboard/Notification", label: "Notification" },
    { to: "/admin/dashboard/AddCategory", label: "Category" },
    { to: "/admin/dashboard/AddLatestUpdate", label: "Add Latest Update" },
  ];
  return (
    <nav className="bg-gray-900 border-b border-gray-700 px-3 sm:px-4 md:px-6 py-3 flex items-center justify-between gap-2 sm:gap-3 sticky top-0 z-40">
      {/* LEFT — Hamburger + Logo */}
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        {/* Hamburger — mobile/tablet only */}
        <button
          onClick={onToggleSidebar}
          className="lg:hidden text-gray-300 hover:text-white p-1.5 rounded-lg hover:bg-gray-700 transition"
        ...
        <Link to="/admin/dashboard" className="flex items-center gap-2">
          <div className="bg-orange-500 text-white font-bold px-2.5 py-1 rounded-lg text-sm flex-shrink-0">ZI</div>
          <span className="text-white font-semibold text-sm hidden sm:block whitespace-nowrap">Admin Panel</span>
        </Link>
      </div>

      {/* CENTER — Quick links (large screens only, scrolls if it ever runs out of room) */}
      <div className="hidden lg:flex items-center gap-4 xl:gap-6 text-sm text-gray-300 flex-1 min-w-0 justify-center overflow-x-auto px-2">
        {navLinks.map((link) => (
          <Link key={link.to} to={link.to} className="hover:text-white transition whitespace-nowrap flex-shrink-0">
            {link.label}
          </Link>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        ...
        <div className="relative">
          ...
          >
            <div className="w-7 h-7 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">A</div>
          ...
          {/* Dropdown */}
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-gray-100 w-52 max-w-[85vw] max-h-[75vh] overflow-y-auto z-50">
              {/* Show all links in dropdown — primary nav on mobile/tablet, backup on desktop */}
              {navLinks.map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
        ...
        <button
          onClick={handleLogout}
          className="hidden sm:block bg-red-500 hover:bg-red-600 text-white text-sm font-medium px-3 md:px-4 py-1.5 rounded-lg transition whitespace-nowrap"
        >
          Logout
        </button>
      </div>
