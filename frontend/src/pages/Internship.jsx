...
const inputBase = {
  ...
  color: "#1a0019", background: "#faf5fa", border: "1.5px solid #e8d8e7",
...
export default function ZInstituteInternship() {
  ...
  const handleSubmit = async () => {
    const { name, email, phone, courseCategory, course, duration } = form;
    if (!name || !email || !phone) { setSubErr("Full name, email, and phone are required."); return; }
    ...
    try {
      const catObj    = categories.find(c => c._id === courseCategory);
      const courseObj = coursesForCat.find(c => c._id === course);
      const res = await fetch(REGISTER_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          type: "internship", name, email, phone,
          degree: form.degree,
          courseCategory,
          courseCategoryName: catObj?.categoryName,
          course,
          courseName: courseObj?.courseName,
          duration,
        }),
      });
      if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d.msg || "Registration failed."); }
      setSubmitted(true);
    } catch (err) {
  ...
  return (
    <>
      {/*
        ── KEY FIX ─────────────────────────────────────────────────────────────
        ALL styles are scoped under .zint-page so nothing leaks to the navbar.
        - No global `* { margin:0; padding:0 }` reset (that was shrinking the navbar)
        - Keyframe names are prefixed with zp- to avoid clashing with other CSS
        ─────────────────────────────────────────────────────────────────────── */}
      <style>{`
        ...
        .zint-page select:disabled    { opacity: 0.55; cursor: not-allowed; }
        .zint-page label {
      ...
      <div className="zint-page">
        ...
        <section style={{
          ...
          <span className="zpill zp-fade-down" style={{ background: PPL, color: DP, border: `1px solid ${PP}44`, marginBottom: 28 }}>
          ...
          <p className="zp-fade-down" style={{ fontSize: 17, color: "#7a4e77", maxWidth: 520, lineHeight: 1.6, marginBottom: 36, fontWeight: 300, animationDelay: "0.2s" }}>
            India's most immersive training & project-based internship program. Real AI projects, real mentors, real results.
          </p>

          <div className="zp-fade-down" style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", animationDelay: "0.3s" }}>
            <button onClick={scrollToForm} className="btn-pri zp-pulse" style={{ fontSize: 16, padding: "14px 32px", borderRadius: 12 }}>
              Register for Internship
            </button>
            <button onClick={() => formRef.current?.scrollIntoView({ behavior: "smooth" })} className="btn-out" style={{ fontSize: 15, padding: "13px 24px", borderRadius: 12 }}>
              View Curriculum ↓
            </button>
          </div>
                    ...
                    {[
                      ...
                      <div key={key}>
                        ...
                        <input type={type} placeholder={placeholder} value={form[key]}
                          onChange={e => set(key, e.target.value)} onFocus={onFocus} onBlur={onBlur} />
                      </div>
                    ))}
