...
export default function CourseDetail() {
  ...
  // Resolve course before any effects that depend on it. Safe to compute
  // even while data is still loading — `course` will just be undefined.
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        // Route matches by targetName (a string), not the course's ObjectId,
        // since the Rating model stores targetName + targetType instead of a ref.
        const res = await safeFetch(`${RATING_URL}/target/${encodeURIComponent(course.courseName)}`);
        const list = res?.data || res?.ratings || [];
        const filtered = list.filter(r =>
          r.targetType === "course" &&
          r.targetName === course.courseName &&
          r.isVisible !== false
        );
        setReviews(filtered);
      } catch (err) {
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-10">
            ...
            <div className="flex-1 order-2 md:order-1">
              ...
              <div className="flex items-center gap-3 mb-7 flex-wrap">
                ...
                <span className="text-sm font-semibold" style={{ color: DARK }}>{avgRating}</span>
                <span className="text-gray-400 text-sm">
                  ({reviews.length > 0 ? `${reviews.length} review${reviews.length > 1 ? "s" : ""}` : "2,400+ students"})
                </span>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          {reviewsLoading ? (
            ...
            <div className="text-center py-16">
              <p className="text-gray-400 text-sm">No reviews yet for this course. Be the first to enroll and share your experience!</p>
            </div>
          ) : (
                ...
                return (
                  <div key={r._id} className="rounded-2xl p-6 border transition-all duration-300"
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed mb-6 italic">
                      "{r.review || "Great course experience!"}"
                    </p>
